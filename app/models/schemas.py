...
class SourceCitation(BaseModel):
    ...
    text: str | None = Field(default=None, description="Raw text snippet of the retrieved chunk or web content")
...
class WorkflowNodeType(str, Enum):
    """Supported node types in the visual workflow builder."""
    ROUTER      = "router"
    RAG         = "rag"
    MEMORY      = "memory"
...
class WorkflowNodeDef(BaseModel):
    ...
    id:       str             = Field(..., description="Unique node ID (generated by React Flow)")
    type:     WorkflowNodeType= Field(..., description="Node type determines what gets executed")
    label:    str             = Field(default="", description="User-facing display label")
...
class WorkflowDefinition(BaseModel):
    """A complete saved workflow — nodes + edges + metadata."""
...
class WorkflowExecutionStep(BaseModel):
    ...
    node_id:     str           = Field(..., description="Which node ran")
    node_type:   str           = Field(..., description="Type label")
    node_label:  str           = Field(default="")
    status:      str           = Field(default="idle",  description="idle | running | done | error")
    output:      dict[str, Any]= Field(default_factory=dict, description="Key-value outputs of the node")
...
class WorkflowExecutionResult(BaseModel):
    """Full execution trace for one workflow run."""
